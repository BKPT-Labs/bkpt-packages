...
/* Run one or more statements inside their own short critical section. Each
   use gets its own scope, so several VA_ATOMIC() calls can sit in the same
   block (VA_CS_ENTER declares a local, so bare back-to-back uses would clash). */
...
typedef struct {
    ...
    /* Stack-usage change detection (see va_taskswitchedout) */
    uint32_t lastStackUsed;                 /* last emitted stack-used value  */
